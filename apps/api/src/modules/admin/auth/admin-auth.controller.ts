...
// Russian-language single error message used for any auth failure (wrong login,
...
export class AdminAuthController {
    ...
    @ApiOperation({ summary: 'Admin login (login + password)' })
    ...
    async login(@Body() dto: LoginDto): Promise<LoginResponseDto> {
        const admin = await this.authService.validateCredentials(dto.login, dto.password);
        if (!admin) {
    ...
    @Throttle({ default: { limit: 5, ttl: 900_000 } })
    @ApiOperation({
        summary: 'Resolve an invite/reset token to the target login + name (public)',
