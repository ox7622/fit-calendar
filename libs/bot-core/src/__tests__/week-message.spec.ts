...
describe('buildWeekMessage', () => {
    it('puts a bold range header on top and lists only non-empty days', () => {
        const { text } = buildWeekMessage(week, 0, 'Europe/Moscow');
    ...
    it('shows the empty-week message but keeps nav buttons', () => {
        const { text, replyMarkup } = buildWeekMessage(emptyWeek, 0, 'Europe/Moscow');
    ...
    it('hides the back arrow at the lower bound', () => {
        const { replyMarkup } = buildWeekMessage(week, -4, 'Europe/Moscow');
        const labels = replyMarkup.inline_keyboard.flat().map((b) => (b as { text: string }).text);
    ...
    it('hides the forward arrow at the upper bound', () => {
        const { replyMarkup } = buildWeekMessage(week, 8, 'Europe/Moscow');
        const labels = replyMarkup.inline_keyboard.flat().map((b) => (b as { text: string }).text);
    ...
    it('encodes target offsets in callback_data', () => {
        const { replyMarkup } = buildWeekMessage(week, 2, 'Europe/Moscow');
    ...
    it('appends the mini-app button when a URL is given', () => {
        const { replyMarkup } = buildWeekMessage(week, 0, 'Europe/Moscow', 'https://app.example.com');
